...
if(is_admin() && did_action('current_screen') && $GLOBALS['plugin_page'] = 'roles'){
	class WPJAM_Roles_Admin{
		...
		public static function insert($data){
			...
			if($result == null){
				return new WP_Error('error', '新建失败，可能重名或者其他原因。');
			}
		...
		public static function update($role, $data){
			...
			if($result == null){
				return new WP_Error('error', '修改失败，可能重名或者其他原因。');
			}
	...
	class WPJAM_Role{
		public static function on_user_profile($profileuser){
			...
			echo wpjam_fields(['capabilities'=> ['title'=>'权限',	'type'=>'mu-text',	'value'=>$capabilities]]);
		}
