...
class WPJAM_Job extends WPJAM_Register{
	...
	public static function autoload(){
		if(self::get_registereds()){
			$object	= wpjam_register_cron('wpjam_scheduled', [
				'recurrence'	=> 'five_minutes',
				'jobs'			=> [self::class, 'get_objects'],
				'weight'		=> true
			]);

			wpjam_set_current_var('wpjam_scheduled', $object);
		}
	}
	...
	public static function load_callback(){
		if(self::get_registereds()){
			wpjam_register_plugin_page_tab('jobs', [
				'title'			=> '作业列表',
				'function'		=> 'list',
				'plural'		=> 'jobs',
				'singular'		=> 'job',
				'primary_key'	=> 'job_id',
				'model'			=> 'WPJAM_Job',
				'summary'		=> '今天已经运行 <strong>'.wpjam_get_current_var('wpjam_scheduled')->get_counter().'</strong> 次'
			]);
		}
	}
}
...
function wpjam_register_cron($name, $args=[]){
	...
	return WPJAM_Cron::create($name, $args);
}
...
wpjam_add_menu_page('wpjam-crons',	[
	...
	'summary'		=> __FILE__,
	'function'		=> 'tab',
	'load_callback'	=> ['WPJAM_Cron', 'load_callback'],
