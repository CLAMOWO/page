jQuery(function($){
					...
					$.each(value, function(sub_key, level){
						...
						if(level.parent != 0){
							if(level.items.length > 0){
								query_select.empty().prepend('<option value="0">所有</option>');
				...
				$('.show_if-'+key).each(function(){
					...
					if(data.compare){
						...
						if($(this).is('option')){
							...
							$(this).find('.show_by').wpjam_show_if();
						}
					}
	...
	$('body').on('click', 'div.mu-fields a.new-item', function(){
		...
		item.insertAfter($(this).parent());
		item.filter('.show_if').wpjam_show_if_class();
		item.find('.show_by').wpjam_show_if();
