...
class WPJAM_Tag extends WPJAM_Attr{
	...
	public function render(){
		...
		if($this->tag){
			...
			if(in_array($this->tag, self::SINGLE_TAGS)){
				$result	.= ' />';
			}else{
				$result	.= '>';
...
class WPJAM_User_Agent extends WPJAM_Args{
	private $user_agent;
	private $_current	= false;	// to do 2023-06-01 去掉

	public function __construct($user_agent=null){
		if(is_null($user_agent)){
			$this->user_agent	= wpjam_get_user_agent();
			$this->_current		= true;
		}else{
	...
	public function __get($name){
		...
		if($this->_current){
			return apply_filters('wpjam_determine_'.$name.'_var', $value, $this->user_agent);	// 使用注册规则来取代
		}
...
class WPJAM_Exception extends Exception{
	...
	public function __construct($errmsg, $errcode=0, Throwable $previous=null){
		if(is_wp_error($errmsg)){
			...
			$errcode	= $this->wp_error->get_error_code();
		}else{
