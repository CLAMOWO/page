		...
		foreach($post_ids as $post_id){
			if(get_post_status($post_id) == 'publish'){
				if(wp_cache_get($post_id, 'wpjam_baidu_zz_notified') === false){
					...
					$urls	.= apply_filters('baidu_zz_post_link', get_permalink($post_id), $post_id)."\n";
				}else{
					if($wp_error){
						return new WP_Error('error', '一小时内已经提交过了');
					}
				}
			}else{
				if($wp_error){
					return new WP_Error('error', '未发布的文章不能同步到百度站长');
				}
			}
		}

		if(!$urls){
			return new WP_Error('error', '没有需要提交到百度站长的链接');
		}
		...
		if($screen->base == 'edit'){
			if(is_post_type_viewable($screen->post_type)){
				wpjam_register_list_table_action('notify_baidu_zz', [
					...
					'bulk_callback'	=> [self::class, 'notify_post'],
					'bulk'			=> true,
					'direct'		=> true
				]);
			}
